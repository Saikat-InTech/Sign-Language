export const six = (ref) => {
  ...
  // 👉 Fold Thumb towards Pinky (X + Z rotation)
  ["mixamorigRightHandThumb1","mixamorigRightHandThumb2","mixamorigRightHandThumb3","mixamorigRightHandThumb4"].forEach(bone => {
    animations.push([bone, "rotation", "z", Math.PI / 4, "+"]);
    animations.push([bone, "rotation", "x", -Math.PI / 3, "+"]);
  });
  ...
  ["Index","Middle","Ring","Pinky","Thumb"].forEach(finger => {
    for (let i = 1; i <= 4; i++) {
      ...
      if(finger === "Thumb") reset.push([`mixamorigRightHand${finger}${i}`, "rotation", "x", 0, "-"]);
    }
  });
